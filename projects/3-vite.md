...
As we discovered in the [ESModules lecture](https://marcylabschool.gitbook.io/swe/mod-3-html-css/esmodules.md), we can't use ESModules with the normal `file://` protocol without running into CORS (cross-origin resource sharing) issues. We need to use the `http://` protocol to enable our files to import values from each other.
...
To deploy this distribution version of your application, check out the article on [How to Deploy on GitHub Pages](https://marcylabschool.gitbook.io/swe/marcy-lab-school-docs/projects/deploying-vite-with-github-pages).
